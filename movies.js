...
// console.log(`posted!`)
let username = document.querySelector(`#posted`).value
let imageUrl = document.querySelector(`#image-url`).value

// console.log(`imageUrl is ${imageUrl}`)
await db.collection(`posted`).add({posted: posted, imageUrl:imageUrl })
  })
  // ⬆️ ⬆️ ⬆️ 
  // End Step 1
  ...
  // </div>
  // ⬇️ ⬇️ ⬇️
  let moviesPosted = await db.collection('movies').get()
  let movies = moviesPosted.docs
  console.log(movies)
  for (let i = 0; i < movies.lenght; i++) {
let movies = movies[i]
let moviesId = movies.id
let posterData = poster.data()
let poster = posterData.poster
let posterImageUrl = posterData.imageUrl

let postLikes = posterData.clicks
...
<div class="w-1/5 p-4 movie-abcdefg1234567">
  ...
  `)
  document.querySelector(`.poster-${posterId} .watch-button`).addEventListener(`watch`, async function() {
    
    let currentNumberMovies = document.querySelector(`.poster-${postId} .watch`).innerHTML
    
    newNumberOfMovies = parseInt(currentNumberOfMovies) + 1
    document.querySelector(`.poster-${posterId} .watch`).innerHTML = newNumberOfLikes

   
    await db.collection(`poster`).doc(posterId).update({watch: firebase.firestore.FieldValue.increment(1) })
  })
  }
  // ⬆️ ⬆️ ⬆️ 
  // End Step 2
